...
The app deploys to Vercel as **two projects from this one repo**, each native to its framework: **backend** (Root Directory `backend`, FastAPI/Python) and **frontend** (Root Directory `frontend`, Next.js). The browser calls the backend **cross-origin** (`NEXT_PUBLIC_API_URL` + `PRELEGAL_CORS_ORIGINS`; Bearer-token auth, no cookies). Full setup steps live in `DEPLOYMENT.md`.

- **Two projects, not one (hard-won).** A single-project design (static-export frontend served by the CDN + FastAPI as a Python function, all under one root `vercel.json`, matching the sibling `pm` project) was attempted first and **abandoned**: on this Vercel project, `/api/*` never reached the function under any routing config (`rewrites` `/api/(.*)`, `rewrites` `/(.*)`, and low-level `routes` were all tried — the function was built but never invoked; every path fell through to the static output). Two native projects sidestep this entirely: the backend project routes `/api/*` to the FastAPI app natively.
- **Backend project.** `backend/index.py` is a thin ASGI shim (prepends `src/` to `sys.path`, re-exports `prelegal_api.main:app`), which Vercel auto-detects as the FastAPI entrypoint and serves for all requests. Deps install from `backend/requirements.txt`; `backend/.vercelignore` hides `pyproject.toml`/`uv.lock` so Vercel uses a plain `pip install`. **`backend/vercel.json` sets `build.env.UV_PYTHON = 3.12`** — this is **required**: Vercel's uv builder ignores `.python-version` and defaults to Python **3.14**, for which `libsql-experimental` has no wheel, so the Rust/maturin source build fails. `maxDuration` is 60s.
- **Frontend project.** A standard Next.js app (Vercel auto-detects and builds it — no static export, no routing config). It only needs `NEXT_PUBLIC_API_URL` (the backend URL, build-time-baked). The legacy `nda/mutual/create` server `redirect()` became a client-side redirect (harmless in normal Next; a leftover from the abandoned static-export attempt).
...
- **Secrets** stay out of the repo (`.env` git-ignored, `.vercel/` in `.gitignore`); all keys are set in the Vercel dashboard. `.env.example` / `.dist.env` document `PRELEGAL_DATABASE_AUTH_TOKEN`.
- **Verification for this ticket:** backend `pytest` (incl. mocked remote-connect tests), a real Turso connectivity smoke test over the Hrana HTTP API, the `backend/index.py` import, and a local `frontend` production build. The live deploy was driven through the Vercel dashboard (browser), fixing the Python-version build failure (`UV_PYTHON=3.12`) along the way.
