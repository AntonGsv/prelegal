...
The app deploys to Vercel as **two Vercel projects from this one repo** — frontend (Root Directory `frontend/`, Next.js preset) and backend (Root Directory `backend/`, FastAPI/Python preset). Full setup steps live in `DEPLOYMENT.md`.

- **Backend as serverless functions.** `backend/index.py` is a thin ASGI shim: it prepends `src/` to `sys.path` and re-exports `prelegal_api.main:app`, which Vercel auto-detects as the FastAPI entrypoint. Deps install from `backend/requirements.txt` (a hand-maintained mirror of `pyproject.toml` runtime deps, **minus uvicorn** — Vercel serves the ASGI app itself). `backend/.vercelignore` excludes `pyproject.toml`/`uv.lock` from the upload so Vercel uses a plain `pip install -r requirements.txt` rather than `uv sync`-ing the project (the repo never installs itself — Docker uses `--no-install-project`, and `pyproject.toml` has no `[build-system]`). Python is pinned to **3.12** via `backend/.python-version` (matches the `libsql-experimental` manylinux wheel). `backend/vercel.json` sets `maxDuration: 60` (Hobby ceiling; the LLM retry policy can exceed it under a sustained provider outage — Pro can raise it, see `DEPLOYMENT.md`).
- **Remote Turso database.** A serverless filesystem is ephemeral, so the DB moves off the local SQLite file to a hosted Turso libSQL database. `connect_database` now accepts an optional `auth_token` and, for a remote (non-`file:`) URL, calls `libsql.connect(url, auth_token=...)`; the token is read from `PRELEGAL_DATABASE_AUTH_TOKEN` (fallback alias `DB_TOKEN`) at the single connect point rather than threaded through every query helper. A local `file:` URL still connects with no token, so **local/Docker dev and tests are unchanged** (sqlite3 fallback intact). Schema init still runs idempotently via the FastAPI `lifespan` hook on cold start.
- **Cross-origin + CORS.** The browser calls the backend's Vercel URL directly: `NEXT_PUBLIC_API_URL` (frontend, build-time-baked) points at the backend, and the frontend origin is allowlisted in `PRELEGAL_CORS_ORIGINS` (backend). Bearer-token auth (no cookies) keeps this simple; only the production frontend origin is allowlisted by default (preview deploys get per-deploy URLs).
- **Secrets** stay out of the repo (`.env` is git-ignored, `.vercel/` added to `.gitignore`); all keys are set in the Vercel dashboard. `.env.example` / `.dist.env` document the new `PRELEGAL_DATABASE_AUTH_TOKEN`.
- **Verification for this ticket:** backend `pytest` (incl. new mocked remote-connect tests), a real Turso connectivity smoke test over the Hrana HTTP API, the `backend/index.py` shim import, and a local `frontend` production build. The live Vercel deploy itself is a manual dashboard step (`DEPLOYMENT.md`).
