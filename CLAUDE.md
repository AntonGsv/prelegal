...
- **Real accounts.** Sign up / sign in with email + password. Passwords are hashed with stdlib PBKDF2-HMAC and stored in a `password_hash` column on the libSQL `users` table (`security.py`, `auth.py`, `database.py`). Auth crypto is deliberately dependency-free (stdlib `hashlib`/`hmac`) to avoid native wheels, matching the libSQL fallback philosophy. Login timing is equalized (a dummy verify runs for unknown emails) to prevent account enumeration, and a concurrent duplicate registration maps to 409, not 500.
- **Bearer-token sessions.** A successful register/login returns a compact HMAC-signed token (a mini-JWT, `security.create_token`) that the frontend keeps in `localStorage` (`auth-storage.ts`) and sends as `Authorization: Bearer`. The backend is stateless — no session store; the token is verified per request (`require_user` dependency). `PRELEGAL_SESSION_SECRET` signs tokens (dev default provided; also wired through `docker-compose.yml` / `.dist.env`).
- **Schema migration.** `init_database` is idempotent: `CREATE TABLE IF NOT EXISTS` for `users`/`documents`, then `_migrate_add_column` ALTERs a pre-PL-7 `users` table (e.g. the persistent Docker `backend-data` volume) in place to add the nullable `password_hash` column — so an existing DB is upgraded without data loss rather than 500-ing on `no such column`. `verify_password` treats a NULL/legacy hash as a clean auth failure.
- **Per-user history.** Generating a PDF also saves `{ slug, fields }` to a `documents` table keyed by user (`document_history.py`). A document is fully reconstructable from slug + fields (template + config derive from the slug), so revisiting re-renders the read-only preview and re-downloads an identical PDF. History lives at `/history`; a saved document opens at `/documents/[slug]/view?id=N` (static per slug, id read client-side — the runner has no source `.md` files). Ownership is enforced server-side (`WHERE id = ? AND user_id = ?`); the save call is fire-and-forget so it never blocks the download.
...
- **Endpoints:** `POST /api/auth/register` (201; 409 dup email; 422 invalid), `POST /api/auth/login` (401), `GET /api/auth/me`; and Bearer-gated `POST|GET /api/documents/history`, `GET /api/documents/history/{id}` (401 without a valid token; 404 for a non-owner).
- **UI polish.** Brand colors are tokenized in `globals.css` (`--brand-*`, `--primary` remapped to brand blue) instead of ad-hoc hex; there is a real marketing landing page (`/`), a header account menu (`ui/avatar` + `ui/dropdown-menu`, base-nova / Base UI), and `next-themes` dark mode (`theme-provider`/`theme-toggle`).

### PL-7 implemented features & build tooling

Features delivered: real email/password accounts, bearer-token sessions, per-user document history (save on generate + `/history` list + read-only re-download view), a draft-review disclaimer on every document, and a SaaS polish pass (brand tokens, landing page, account menu, dark mode). New modules: backend `security.py` / `auth.py` / `document_history.py`; frontend `auth-form`, `document-history`, `document-view`, `theme-provider`/`theme-toggle`, `ui/avatar`, `ui/dropdown-menu`, `use-auth`.

Tooling used to build it (repeat for future tickets): **Atlassian (Jira) MCP** to read the ticket and move it through To Do → In Review → Done; the **feature-dev 7-step process** with `code-explorer` / `code-reviewer` subagents for exploration and a 3-dimension review; the **Cerebras skill** for the LLM design (unchanged here); **pytest** (backend), **Vitest** (frontend unit), and **Playwright** (E2E) for testing; and the **GitHub MCP** to open the PR (`feature/PL-7-multi-user-and-polish` → `main`, merged as PR #9).

### Vercel deployment (PL-8 decisions)

The app deploys to Vercel as **one project for the whole repo** (Root Directory = repository root). The Next.js frontend is built as a **static export** and served by Vercel's CDN; the FastAPI backend runs as a **Python serverless function** at `api/index.py`, and `/api/*` is rewritten to it — so the app is **single-origin, no CORS**. All wiring is in the root `vercel.json`. Full setup steps live in `DEPLOYMENT.md`.

- **Frontend as static export.** `next.config.ts` enables `output: "export"` **only** when the `STATIC_EXPORT` env var is set, which the Vercel `buildCommand` (`cd frontend && npm install && STATIC_EXPORT=1 npm run build`) does. Without the flag, the frontend builds in normal server mode, so `docker compose up` (`next start`) is unaffected. All pages are already static/SSG; the one server `redirect()` (legacy `nda/mutual/create`) became a client-side redirect for export compatibility. `frontend/.env.production` sets `NEXT_PUBLIC_API_URL=` (empty → same-origin `/api/...`); a Docker build overrides it via its build ARG, so local cross-origin dev still works.
- **Backend as a serverless function.** Root `api/index.py` puts `backend/src` on `sys.path` and re-exports `prelegal_api.main:app`; the backend source is bundled via `functions."api/index.py".includeFiles: "backend/**"`. The rewrite `/api/(.*) → /api/index` funnels API calls to the function, which receives the original path so FastAPI's `/api/...` routes match. Deps install from the root `requirements.txt` (runtime deps mirrored from `pyproject.toml`, minus uvicorn); Python pinned to **3.12** via root `.python-version` (matches the `libsql-experimental` manylinux wheel). The FastAPI app itself is unchanged — the CDN serves static, so no static-file mount is needed.
- **Remote Turso database.** A serverless filesystem is ephemeral, so the DB moves off the local SQLite file to a hosted Turso libSQL database. `connect_database` accepts an optional `auth_token` and, for a remote (non-`file:`) URL, calls `libsql.connect(url, auth_token=...)`; the token is read from `PRELEGAL_DATABASE_AUTH_TOKEN` (fallback alias `DB_TOKEN`) at the single connect point rather than threaded through every query helper. A local `file:` URL still connects with no token, so **local/Docker dev and tests are unchanged** (sqlite3 fallback intact). Schema init runs idempotently via the FastAPI `lifespan` hook on cold start.
- **Secrets** stay out of the repo (`.env` git-ignored, `.vercel/` in `.gitignore`); all keys are set in the Vercel dashboard. `.env.example` / `.dist.env` document `PRELEGAL_DATABASE_AUTH_TOKEN`. Same-origin means no `PRELEGAL_CORS_ORIGINS`/`NEXT_PUBLIC_API_URL` needed on Vercel.
- **History:** this single-project design (matching the sibling `pm` project's pattern) superseded an initial two-project design (separate frontend/backend Vercel projects, cross-origin + CORS) from earlier in PL-8; the `connect_database` Turso change carried over unchanged.
- **Verification for this ticket:** backend `pytest` (incl. mocked remote-connect tests), a real Turso connectivity smoke test over the Hrana HTTP API, the root `api/index.py` import, and local `frontend` builds in **both** modes (server + `STATIC_EXPORT` export). The live Vercel deploy itself is a manual dashboard step (`DEPLOYMENT.md`).
