...
// Standard Terms from CommonPaper Mutual NDA (CC BY 4.0)
...
export function renderCoverPage(doc: jsPDF, data: NdaFormData): void {
  ...
  doc.setFontSize(18);
  doc.text("MUTUAL NON-DISCLOSURE AGREEMENT", pageWidth / 2, 30, {
    align: "center",
  });
  ...
  doc.setFontSize(11);
  doc.text(`Effective Date: ${formatDate(data.effectiveDate)}`, margins, 50);

  // MNDA Term
  doc.text(`Term: ${data.ndaTerm}`, margins, 60);
  doc.text(`Confidentiality Term: ${data.confidentialityTerm}`, margins, 70);

  // Purpose
  doc.setFont("helvetica", "bold");
  doc.text("Purpose:", margins, 85);
  doc.setFont("helvetica", "normal");
  const purposeLines = doc.splitTextToSize(data.purpose, contentWidth);
  doc.text(purposeLines, margins, 95);
  let yPos = 95 + purposeLines.length * 6 + 15;

  // Party A
  doc.setFont("helvetica", "bold");
  doc.text("Party A:", margins, yPos);
  yPos += 8;
  doc.setFont("helvetica", "normal");
  doc.text(`Company: ${data.partyA_companyName}`, margins, yPos);
  yPos += 6;
  doc.text(`Address: ${data.partyA_address}`, margins, yPos);
  yPos += 6;
  doc.text(`Representative: ${data.partyA_representative}`, margins, yPos);
  yPos += 6;
  doc.text(`Email: ${data.partyA_email}`, margins, yPos);
  yPos += 15;

  // Party B
  doc.setFont("helvetica", "bold");
  doc.text("Party B:", margins, yPos);
  yPos += 8;
  doc.setFont("helvetica", "normal");
  doc.text(`Company: ${data.partyB_companyName}`, margins, yPos);
  yPos += 6;
  doc.text(`Address: ${data.partyB_address}`, margins, yPos);
  yPos += 6;
  doc.text(`Representative: ${data.partyB_representative}`, margins, yPos);
  yPos += 6;
  doc.text(`Email: ${data.partyB_email}`, margins, yPos);
  yPos += 15;

  // Governing Law
  doc.setFont("helvetica", "bold");
  doc.text("Governing Law and Jurisdiction:", margins, yPos);
  yPos += 8;
  doc.setFont("helvetica", "normal");
  doc.text(`Governing Law: ${data.governingLaw}`, margins, yPos);
  yPos += 6;
  doc.text(`Jurisdiction: ${data.jurisdiction}`, margins, yPos);
  yPos += 20;

  // Signature Lines
  doc.setFont("helvetica", "bold");
  doc.text("Signatures:", margins, yPos);
  yPos += 15;

  doc.setFont("helvetica", "normal");
  doc.text("_____________________________", margins, yPos);
  doc.text("Party A Representative", margins, yPos + 5);
  doc.text("Date: _______________", margins + 80, yPos);

  doc.text("_____________________________", margins, yPos + 20);
  doc.text("Party B Representative", margins, yPos + 25);
  doc.text("Date: _______________", margins + 80, yPos + 20);
}
